...
func _process(delta):
	if has_node("p1") and has_node("p2"):
		var p1 = $p1
		var p2 = $p2
		p1.peut_bouger = combat_actif
		p2.peut_bouger = combat_actif
...
func afficher_splashart_ulti(p_id, image_splash):
	splash_art.texture = image_splash
	splash_art.modulate.a = 0
	splash_art.visible = true
	var tw = create_tween()
	tw.tween_property(splash_art, "modulate:a", 1.0, 0.1)
	tw.tween_interval(0.8)
	tw.tween_property(splash_art, "modulate:a", 0.0, 0.1)
	tw.finished.connect(func(): splash_art.visible = false)
...
func demarrer_sequence_intro():
	if compte_a_rebours > 0:
		...
		combat_actif = true
		demarrer_chrono()
		get_tree().create_timer(1.0).timeout.connect(func(): label_countdown.visible = false)
