...
func _ready():
	...
	%BoutonAttaque.pressed.connect(filtrer_champions.bind("Eleve Guerrier",  %BoutonAttaque))
	%BoutonDefense.pressed.connect(filtrer_champions.bind("Défense", %BoutonDefense))
...
func _changer_scene():
	if selection_p1 and selection_p2:
		...
		get_tree().change_scene_to_file("res://transi.tscn")
